...
class TimeWidgetRefSyle extends React.Component {
  constructor(props) {
    super(props);
  ...
  //     if ( nextProps.value ) {
  //         console.log('va', nextProps.value, sanitizeValueForBrowserInput( nextProps.value ));
  //         this.inputRef.current.value = sanitizeValueForBrowserInput( nextProps.value );
  //     }
  // }
  ...
  render() {
    ...
    return (
      <BaseInputWithRef
        ...
        // ref={this.inputRef}
        onChange={value => {
          onChange(ensureSecondsArePresentInTimeFormat(value));
        }}
        step={step ? step : null}
      />
    );
...
if (process.env.NODE_ENV !== "production") {
  TimeWidgetRefSyle.propTypes = {
    value: PropTypes.string,
  };
}

export default TimeWidgetRefSyle;
